...
void Ay38912<nc>::setRegister(CC cc, uint r, uint8 value, WritePortProc& callback)
{
	...
	if (r == 7)
	{
		...
		if (t & 0x40 && ay_reg[14] != 0xff) callback(ccx_now, 0, value & 0x40 ? ay_reg[14] : 0xff);
		if (t & 0x80 && ay_reg[15] != 0xff) callback(ccx_now, 1, value & 0x80 ? ay_reg[15] : 0xff);
	}
	else if (r >= 14)
	{
		if (ay_reg[r] != value && ay_reg[7] & (1 << (r & 7))) callback(ccx_now, r & 1, value);
	}
...
void Ay38912<nc>::run_up_to_cycle(CCx ccx_end) noexcept
{
	...
	while (true)
	{
		...
		if (envelope.when < ccx_when)
		{
			...
			else if (envelope.direction) envelope.when += 1024 << ccx_fract_bits;
			else envelope.when += 0x3fffff00;
		}

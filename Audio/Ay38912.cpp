...
template<uint nc>
Ay38912<nc>::Ay38912(float ay_frequency, AyStereoMix mix, float volume) noexcept :
	stereo_mix(nc == 2 ? mix : Mono),
	ccx_per_sample(int(ay_frequency * (1 << ccx_fract_bits) / hw_sample_frequency))
{
	setVolume(volume);
	reset();
}
...
template<uint nc>
void Ay38912<nc>::setClock(float frequency) noexcept
{
	...
	ccx_per_sample = int(frequency * (1 << ccx_fract_bits) / hw_sample_frequency);
	setVolume(volume);
}
...
template<uint nc>
uint Ay38912<nc>::getAudio(AudioSample<nc>* buffer, uint num_frames) noexcept
{
	output_buffer = buffer;
	run_up_to_cycle(ccx_now + int(num_frames) * ccx_per_sample);
	return num_frames;
}

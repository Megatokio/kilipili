...
signed long mad_timer_count(mad_timer_t timer, enum mad_units units)
{
	switch (units)
	{
	case MAD_UNITS_HOURS: return timer.seconds / 60 / 60;

	case MAD_UNITS_MINUTES: return timer.seconds / 60;

	case MAD_UNITS_SECONDS: return timer.seconds;
	...
	case MAD_UNITS_75_FPS:
		return timer.seconds * (signed long)units +
			   (signed long)scale_rational(timer.fraction, MAD_TIMER_RESOLUTION, units);
	...
	case MAD_UNITS_59_94_FPS: return (mad_timer_count(timer, mad_units(-units)) + 1) * 1000 / 1001;
	}
...
void mad_timer_string(
	mad_timer_t timer, char* dest, const char* format, enum mad_units units, enum mad_units fracunits,

...
//	trace(__func__);
//
//	super::setSampleRate(new_sample_frequency);
//}
...
//		}
//
//		CC cc_buffer_end = ay.audio_buffer_start(buffer, num_frames);
...
//	{
//		ay.audio_buffer_start(buffer, num_frames);
//		ay.audio_buffer_end();
//		return num_frames;
//	}
//	else
...
int YMMusicPlayer::run() noexcept
{
	...
	try
	{
		...
		else if (next_file)
		{
			...
			if (!is_live) AudioController::getRef().addAudioSource(this);
			is_live = true;
		}
		else if (ymmusic_dir)
		{
		...
		else if (is_live)
		{
			...
			{
				is_live = false;
				AudioController::getRef().removeAudioSource(this);
			}
		}

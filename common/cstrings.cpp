...
cptr rfind(cstr target, cstr search) noexcept
{
	...
	if (s_len == 0) return target + t_len;
...
str usingstr(cstr fmt, va_list va)
{
	...
	if (size < sizeof(bu)) return dupstr(bu);
	...
	if (q)
		for (;;)
		{
			...
			if (is_oct_digit(c))
			{
				...
				if (c == 0) errno = EILSEQ; //brokenescapecode;
			}
			...
			else
			{
				...
				else errno = EILSEQ;   //brokenescapecode; // else self-escaped char
			}
...
str fromutf8str(cstr qstr) // sets errno
{
	...
	while (*q)
	{
		...
		if (is_utf8_fup(c1))
		{
			errno = EILSEQ; //unexpectedfup;
			continue;
		}
		if (uint8(c1) >= 0xc4)
		{
			*z++  = '?';
			errno = EILSEQ; //notindestcharset;
			continue;
		}
		if (*q == 0 || !is_utf8_fup(c2 = *q))
		{
			*z++  = '?';
			errno = EILSEQ; //truncatedchar;
			continue;
		}
		q++;
		*z++ = char((c1 << 6) + (c2 & 0x3f));
...
{
	if (!b) return true; // leerer suchstring

...
static Char const* key_table[2][2] =  // [w/shift][w/alt]
{
	{	key_table_us,       // solo
		key_table_us		// alt
	},
...
static int calc_char(bool down, Modifiers modifiers, HIDKey hid_key)
{
	...
	int c = hid_key < KEY_TRANSLATION_TABLE_SIZE ?
				key_table[modifiers&SHIFT?1:0][modifiers&ALT?1:0][hid_key] : 0;
...
static bool find(HIDKey key, const HIDKey keys[6])
{
	return key != 0 && memchr(keys,key,6) != nullptr;
}

KeyEvent::KeyEvent(bool d, Modifiers m, HIDKey key) noexcept :
	down(d), modifiers(m), key(key), ucs2char(UCS2Char(calc_char(d,m,key))) {}
...
static void handle_key_event(const KeyboardReport& new_report, const KeyboardReport& old_report, bool down, void(*handler)(bool,Modifiers,HIDKey))
{
	for (uint i=0; i<6; i++)
	{

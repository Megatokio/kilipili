...
{
	Error e = ferror(file) ? dupstr(strerror(errno)) : END_OF_FILE;
	clearerr(file);
	throw e;
}
...
ADDR StdFile::getSize() const noexcept
{
	...
	if (!ferror(file))
	{
		if constexpr (sizeof(ADDR) >= sizeof(fsize)) return ADDR(fsize);
...
ADDR StdFile::getFpos() const noexcept
{
	...
	if (!ferror(file))
	{
		if constexpr (sizeof(ADDR) >= sizeof(fpos)) return ADDR(fpos);

...
I2cEeprom::I2cEeprom(
	...
	BlockDevice(total_size >> ss, log2(1), ss, 0, flags),
	i2c_port(i2c_port),
	i2c_addr(i2c_addr),
	erased_byte(0xff), // 0x00 is good as well..
	write_time_us(uint16(std::min(write_time_ms << 10, 65535u)))
{
...
a:
	...
	if (n == int(size + 2))
	{
		if (size) { write_started_us = time_us_32(); }
		return;
	}
	if (n != 0) throw HARD_WRITE_ERROR;
	if (time_us_32() - write_started_us < write_time_us) goto a;

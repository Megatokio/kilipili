...
SDCard* SDCard::defaultInstance() // static
{
	...
	constexpr uint8 rx	= PICO_DEFAULT_SPI_RX_PIN;
	constexpr uint8 cs	= PICO_DEFAULT_SPI_CSN_PIN;
	constexpr uint8 clk = PICO_DEFAULT_SPI_SCK_PIN;
	constexpr uint8 tx	= PICO_DEFAULT_SPI_TX_PIN;

	static SDCard sdcard(rx, cs, clk, tx);
	if (sdcard.rc == 0) sdcard.retain(); // never destroy!
	return &sdcard;
#endif
}

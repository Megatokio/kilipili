...
struct VideoMode
{
	...
	AttrHeight attrheight : 5;

	constexpr VideoMode() = default;

	constexpr VideoMode(
		ScreenSize ss, ColorDepth cd, AttrMode am = AttrMode::attrmode_none, AttrWidth aw = AttrWidth::attrwidth_none,
		AttrHeight ah = AttrHeight::attrheight_none) :
		screensize(ss),
		attrmode(am),
		attrwidth(aw),
		colordepth(cd),
	...
	constexpr uint		get_bitsperpixel() const noexcept
	{
		return attrmode == AttrMode::attrmode_none ? 1 << colordepth : 1 << attrmode;
	}
};

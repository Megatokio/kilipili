...
{
	importFile(infile, verbose);
	cstr basename = basename_from_path(infile);
	if (!outdir) outdir = directory_from_path(infile);
	if (ne(outdir, "") && !endswith(outdir, "/")) outdir = catstr(outdir, "/");

	if (what & WAV)
	{
		uint32 size = exportWavFile(catstr(outdir, basename, ".wav"));
	...
	if (what & YMRF)
	{
		uint32 size = exportYMRegisterFile(catstr(outdir, basename, ".ymrf"));
		if (verbose) printf(".ymrf size: %d\n", size);
	}
	if (what & YM)
	{
		uint32 size = exportYMFile(catstr(outdir, basename, ".unc.ym"));
		if (verbose) printf(".ym   size: %d\n", size);
	}
	if (what & RSRC)
	{
		uint32 size = exportRsrcFile(catstr(outdir, basename, ".rsrc"), catstr(basename, ".ymrf"));
		if (verbose) printf(".rsrc csize: %d \n", size);
	}
}

void YMFileConverter::convertDir(cstr basedir, bool recursive, bool verbose)
{
	convert_dir(basedir, "", recursive, verbose);
}

void YMFileConverter::convert_dir(cstr basedir, cstr subdir, bool recursive, bool verbose)
{
	if (!endswith(basedir, "/")) basedir = catstr(basedir, "/");
	if (ne(subdir, "") && !endswith(subdir, "/")) subdir = catstr(subdir, "/");
	cstr dirpath = catstr(basedir, subdir);
	...
	{
		if (de->d_type == DT_DIR && recursive && ne(de->d_name, ".") && ne(de->d_name, ".."))
			convert_dir(basedir, catstr(subdir, de->d_name), recursive, verbose);
		if (de->d_type != DT_REG) continue; // not a file
		if (ne(lowerstr(extension_from_path(de->d_name)), ".ym")) continue;

		try
		{
			if (verbose) printf("-----------------------------\n");
			if (verbose) printf("converting %s%s\n", subdir, de->d_name);
			else debugstr("converting %s%s\n", subdir, de->d_name);

			convert(catstr(dirpath, de->d_name), dirpath, verbose);
		}
		catch (Error e)
		{
			printf("*** %s\n", e);

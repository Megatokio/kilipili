...
void EncodedImage::write_to_file(cstr fpath)
{
	...
	file.write<uint8>(uint8(abs_codes.num_codes));
	file.write<uint8>(uint8(256 - rel_codes.first_code)); // 0,0 => 256 abs codes (actually real indexed color)
	file.write(ctab, sizeof(ctab));						  // uint16 ctab[256]
